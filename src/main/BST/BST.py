...
class BST:
    ...
    LAYOUT_CONFIG = {"vertex_spacing": (0.0001, .3)}
    VERTEX_CONF = {"radius": 0.45, "color": WHITE, "stroke_width": 4.0}
    ...
    def add(self, val):
        ...
        if (currHeight < self.height and self.height > 1):
            ...
            self.scene.play(self.scene.camera.frame.animate.scale((1.000000000000000000000001 * self.height) / 2.0))
            self.shiftTree(self.root.left, shift, 'L', currHeight - 1)
            self.shiftTree(self.root.right, shift, 'R', currHeight - 1)
    ...
    def _addHelper(self, curr, val, parent, text, direction, depth):
        if (curr == None):
            ...
            animateTraverse(self.scene, self, curr, self.c, text, parent)
            curr.right = self._addHelper(curr.right, val, curr, text, 'R', depth - 1)
        elif (curr.val > val):
            animateTraverse(self.scene, self, curr, self.c, text, parent)
            curr.left = self._addHelper(curr.left, val, curr, text, 'L', depth - 1)
        if (parent != None):
            self.g.edges[(parent.val, curr.val)].set_color(WHITE)
        self.g.vertices[curr.val].stroke_color = WHITE                           
    ...
    def shiftTree(self, curr, shift, direction, depth):
        if (curr == None):
            return
        if (shift <= 0):
            shift = 1
        ...
        newShift = 2 ** (depth - 1)
        totalShift1 = shift + newShift
        totalShift2 = shift - newShift
        if (newShift < 1):
            totalShift1 = shift
            totalShift2 = shift

        if (direction == 'L'):
            self.shiftTree(curr.left, totalShift1, 'L', depth - 1)
            self.shiftTree(curr.right, totalShift2, 'L', depth - 1)
        elif (direction == 'R'):
            self.shiftTree(curr.left, totalShift2, 'R', depth - 1)
            self.shiftTree(curr.right, totalShift1, 'R', depth - 1)
